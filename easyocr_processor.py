...
def main():
    ...
    for i in range(0, len(tiff_processed_files), batch_size):
        batch_files = tiff_processed_files[i:i + batch_size]
        logging.info(f"Processing TIFF batch {i//batch_size + 1} ({len(batch_files)} pages)")
        async def process_tiff_batch():
            async with aiohttp.ClientSession() as session:
                tasks = []
                for file_path, pages_data in batch_files:
                    new_uuid = uuid.uuid4()
                    utc_now = datetime.datetime.utcnow()
                    timestamp = utc_now.timestamp()
                    uuid_str = f"{new_uuid}_{timestamp}"
                    ...
                    language = detect_language(pages_data[0])  # 첫 페이지로 언어 감지
                    if language is None:
                        language = "unknown"
                        
                    task = asyncio.ensure_future(
                        send_tiff_pages_to_server(session, ocr_server_ip, ocr_server_port, uuid_str, pages_data, language)
                    )
                    tasks.append(task)
                ...
                for task in asyncio.as_completed(tasks):
                    try:
                        ...
                        logging.error(f"Error processing TIFF response: {str(e)}")
        
        asyncio.run(process_tiff_batch())
        logging.info(f"TIFF batch {i//batch_size + 1} completed")
        # asyncio.run(send_image_to_server(ocr_server_ip, ocr_server_port, batch_files, json_data))

    logging.info(f"Total normal image files : {len(other_image_files)}")

    # 일반 이미지 처리
    filtered_files, big_size_files = filter_large_images(other_image_files, json_data)
    for i in range(0, len(filtered_files), batch_size):
        batch_files = filtered_files[i:i + batch_size]
        logging.info(f"Processing normal image batch {i//batch_size + 1} ({len(batch_files)} files)")
        asyncio.run(send_image_to_server(ocr_server_ip, ocr_server_port, batch_files, json_data))

    logging.info(f"Regular image batch {i//batch_size + 1} completed")
