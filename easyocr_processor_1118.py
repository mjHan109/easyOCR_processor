...
def resize_image_in_memory(file_path, json_data):
    ...
    try:
        ...
        if width <= 0 or height <= 0:
            logging.error(f"유효하지 않은 이미지 크기: {width}x{height}, 파일: {file_path}")
            return None
        ...
        if width > max_width and height > max_height:
            ...
            if scale <= 0:
                logging.error(f"유효하지 않은 스케일 값: {scale}, 파일: {file_path}")
                return None
            ...
            if new_width <= 0 or new_height <= 0:
                logging.error(f"유효하지 않은 새 이미지 크기: {new_width}x{new_height}, 파일: {file_path}")
                return None
        ...
        if success:
            ...
            logging.error(f"이미지 인코딩 실패: {file_path}")
            return None
    except Exception as e:
...
def check_memory_and_resize(file_path, max_pixels, json_data):
    ...
    try:
        ...
        # PNG 인코딩 파라미터 설정 - ICC 프로파일 제거
        encode_params = [
            cv2.IMWRITE_PNG_COMPRESSION, 9,  # 최대 압축
            cv2.IMWRITE_PNG_STRATEGY, cv2.IMWRITE_PNG_STRATEGY_DEFAULT,
            cv2.IMWRITE_PNG_BILEVEL, 0  # ICC 프로파일 제거
        ]
...
def process_heic_file(file_path):
    ...
    try:
        ...
        logging.error(f"HEIC 파일 처리 중 오류 발생: {file_path}, {str(e)}")
        return None
...
def filter_large_images(image_files, json_data):
    ...
    for file_path in image_files:
        try:
            ...
            # 크기 제한 초과 시 바로 big_size_files에 추가
