...
def read_and_preprocess_image(file_path, json_data):
    """이미지 파일을 읽고, 크기를 확인하며 필요한 경우 리사이즈"""
    # 파일 열기
    img = None
    if file_path.lower().endswith('.gif'):
        img = convert_gif_to_png(file_path)
    elif file_path.lower().endswith(('.heic', '.heif')):
        img = convert_heic_to_png(file_path)
    else:
        img = cv2.imread(file_path)
    ...
    return img_bytes.tobytes() if success else None, width, height
...
def main():
    ...
    if tif_mode and tiff_files:
        ...
        for tiff_file in tiff_files:
            try:
                for frame in process_tiff_file(tiff_file, json_data):
                ...
                save_as_json(tiff_file, {}, json_data, success=False)
                tiff_failure += 1
