...
def remove_text_chunks(file_path):
    """
    PNG 파일에서 텍스트 청크 제거 함수
    Parameters:
        file_path (str): 파일 경로
    Returns:
        bytes: 텍스트 청크가 제거된 PNG 데이터
    """
    try:
...
def is_valid_image(file_path):
    """
    파일이 유효한 이미지인지 검증하는 함수
    Parameters:
        file_path (str): 파일 경로
    Returns:
        bool: 유효한 이미지이면 True, 아니면 False
    """
    try:
        with Image.open(file_path) as img:
        ...
        logging.error(f"Invalid or corrupted image file: {file_path}, Error: {str(e)}")
        return False

# 2024.11.20 추가
def is_valid_image_data(image_data):
    """
    BytesIO 데이터가 유효한 이미지인지 확인
    Parameters:
        image_data (bytes): 이미지 데이터
    Returns:
        bool: 유효한 이미지 데이터이면 True, 아니면 False
    """
    try:
        ...
        logging.error(f"Invalid image data in BytesIO object: {str(e)}")
        return False
