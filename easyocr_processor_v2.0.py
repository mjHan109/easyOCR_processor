...
class TiffImageProcessor:
    ...
    def _process_tiff(self):
        """TIFF 파일을 처리하여 크기와 PNG 데이터를 저장"""
        try:
            with Image.open(self.file_path) as img:
                ...
                img_byte_arr.seek(0)
                self.png_data = img_byte_arr.getvalue()
        except Exception as e:
            logging.error(f"TIFF 처리 중 오류 발생 {self.file_path}: {str(e)}")
...
def get_or_create_tiff_processor(file_path):
    """TIFF 프로세서를 가져오거나 생성"""
    if file_path not in tiff_cache:
        tiff_cache[file_path] = TiffImageProcessor(file_path)
    return tiff_cache[file_path]
...
def process_image_to_grayscale(image_data: bytes) -> bytes:
    ...
    try:
        with Image.open(io.BytesIO(image_data)) as image:
            ...
            if image.format and image.format.lower() in ['tiff', 'tif']:
                try:
                    image.seek(0)
                except EOFError:
                    pass
...
def check_memory_and_resize(file_path, max_pixels, json_data):
    ...
    try:
        ...
        if ext in ['.tiff', '.tif']:
            processor = get_or_create_tiff_processor(file_path)
            image_data = processor.get_png_data()
        else:
            with open(file_path, 'rb') as f:
                image_data = f.read()
...
async def send_image(session, url, uuid_str, image_data, language):
    ...
    try:
        ...
        try:
            # logging.info(f"Send image to server {form_data.get_payload()}\n\n\n\n\n")
            async with session.post(url, data=form_data) as response:
                try:
                    ...
                    logging.error(f"response error {uuid_str}: {str(e)}")
                    return uuid_str, None
        except Exception as e:
            logging.error(f"Send image to server error {uuid_str}: {str(e)}")
            return uuid_str, None
    except Exception as e:
