...
def resize_image_in_memory(file_path, img, width, height, json_data, exception_list):
    ...
    try:
        max_width = json_data['ocr_info']['max_width']
        max_height = json_data['ocr_info']['max_height']
        max_pixels = json_data['ocr_info']['max_pixel_limit']
        # 리사이즈가 필요한지 확인
        if width <= max_width and height <= max_height:
        ...
        if new_width * new_height > max_pixels:
            # logging.error(f"리사이즈된 이미지도 제한을 초과함 ({new_width}x{new_height} > {max_pixels} pixels): {file_path}")
            exception_list.append(file_path)
            return None
        # 이미지 리사이즈
        img = img.resize((new_width, new_height), Image.LANCZOS)
...
def process_image_batch(args):
    ...
    이미지 배치를 처리하는 함수
    """
    batch, json_data, ocr_server_ip, ocr_server_port, batch_id = args
    results = []
    
    # 각 프로세스에서 새로운 이벤트 루프 생성
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    
    try:
        # 비동기 처리를 동기적으로 실행
        results = loop.run_until_complete(process_batch_async(batch, json_data, ocr_server_ip, ocr_server_port, batch_id))
        return results
    except Exception as e:
...
async def process_file_batch(batch, json_data, ocr_server_ip, ocr_server_port, uuid_map):
    """
    각 프로세스에서 파일 배치를 처리하는 함수
    """
    exception_list = []
    processed_results = []
    ...
    for file_path in batch:
        try:
            # 이미지 크기 확인 및 리사이즈 필요 여부 판단
            with Image.open(file_path) as img:
                width, height = img.size

                # 리사이즈 필요한 경우 수행
                if width * height > json_data['ocr_info']['max_pixel_limit']:
                    resized_image = resize_image_in_memory(file_path, img, width, height, json_data, exception_list)
                    if not resized_image:
                        raise ValueError(f"리사이즈 실패: {file_path}")
                else:
                    resized_image = process_image(img)

            # UUID 가져오기
            uuid_str = uuid_map[file_path]

            # 서버 전송 준비
            language = detect_language(resized_image)
            form_data = aiohttp.FormData()
            form_data.add_field('file', resized_image)
            form_data.add_field('language', language)
            form_data.add_field('uuid_str', uuid_str)
            ...
            url = f"http://{ocr_server_ip}:{ocr_server_port}/ocr/"
            async with aiohttp.ClientSession() as session:
                response = await send_single_image(session, url, form_data, file_path, uuid_str)

            # 결과 저장
            processed_results.append(response)

        except Exception as e:
            logging.error(f"파일 처리 실패: {file_path}, 오류: {str(e)}")
            exception_list.append(file_path)

    return processed_results, exception_list


def process_batches_in_parallel(image_files, json_data, uuid_map, ocr_server_ip, ocr_server_port):
    """
    파일 배치를 멀티프로세싱으로 처리하는 함수
    """
    batch_size = json_data['ocr_info']['batch_size']
    num_processes = json_data['ocr_info']['process_count']
    ...
    with Pool(processes=num_processes) as pool:
        process_args = [(batch, json_data, ocr_server_ip, ocr_server_port, uuid_map) for batch in batches]
        results = pool.starmap(process_file_batch, process_args)

    # 결과 취합
    all_results = []
    all_exceptions = []
    for result, exceptions in results:
        all_results.extend(result)
        all_exceptions.extend(exceptions)

    return all_results, all_exceptions
...
async def process_batch_async(batch, json_data, ocr_server_ip, ocr_server_port, batch_id):
    """
    배치를 비동기적으로 처리하는 함수
    """
    results = []
    async with aiohttp.ClientSession() as session:
        ...
        for file_info in batch:
            if isinstance(file_info, tuple):
                file_path, image_data = file_info
            else:
                file_path = file_info
                image_data = None
                
            task = process_single_file(session, file_path, image_data, ocr_server_ip, ocr_server_port, json_data)
            tasks.append(task)
        ...
        for result in batch_results:
            if isinstance(result, Exception):
                logging.error(f"파일 처리 중 오류 발생: {str(result)}")
...
async def process_single_file(session, file_path, image_data, ocr_server_ip, ocr_server_port, json_data):
    """
    단일 파일을 처리하는 함수
    """
    try:
        ...
        if image_data is not None:
            form.add_field('file', image_data, filename=os.path.basename(file_path))
        else:
        ...
        async with session.post(url, data=form, timeout=30) as response:
            if response.status == 200:
                result = await response.json()
                # JSON 파일 생성
                save_as_json(file_path, uuid_str, result, json_data, success=True)
                return True
            else:
                logging.error(f"OCR 서버 오류 응답 - {file_path}: {response.status}")
                save_as_json(file_path, uuid_str, None, json_data, success=False)
                return False
...
def main():
    ...
    try:
        ...
        logging.info(f"OCR START {current_time}, {ocr_ver} ver")
    ...
    exception_list = []
    # 이미지 필터링 및 처리
    logging.info(f"이미지 필터링 시작")
    filtered_files, big_size_files, exception_list = filter_large_images(image_files, json_data, exception_list)
    logging.info(f"이미지 필터링 완료")
    ...
    try:
        logging.info(f"배치 처리 시작")
        results, exceptions = process_batches_in_parallel(image_files, json_data, uuid_map, ocr_server_ip, ocr_server_port)
        end_time = time.time()
        ...
        # logging.info(f"분석 성공 파일: {success_count}개, 분석 실패 파일: {exception_list}")
        # 최종 결과 출력
        logging.info(f"총 파일 처리 완료: {len(results)}개 성공, {len(exceptions)}개 실패")
        # logging.info(f"총 소요 시간: {end_time - start_time:.2f}초")

    except Exception as e:
        logging.error(f"프로세스 중 오류 발생: {str(e)}")
