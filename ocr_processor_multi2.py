...
def resize_image_in_memory(img, width, height, json_data):
    """
    이미지를 메모리에서 리사이즈하는 함수
    """
    try:
        ...
        try:
            ...
            with Image.open(file_path) as img:
                ...
                if width * height > json_data['ocr_info']['max_pixel_limit']:
                    resized_image = resize_image_in_memory(img, width, height, json_data)
...
def process_batch(args):
    ...
    배치를 처리하는 동기 함수
    """
    batch_data, json_data, ocr_server_ip, ocr_server_port = args
    ...
    try:
        # 비동기 처리를 동기적으로 실행
        return loop.run_until_complete(process_batch_async(batch_data, json_data, ocr_server_ip, ocr_server_port))
    finally:
        loop.close()

async def process_batch_async(batch_data, json_data, ocr_server_ip, ocr_server_port):
    """
    배치를 비동기적으로 처리하는 함수
    """
    results = []
    async with aiohttp.ClientSession() as session:
        for file_path, image_data in batch_data:
            try:
                ...
                result = await process_single_file(session, file_path, image_data, uuid_str, ocr_server_ip, ocr_server_port)
                ...
                logging.error(f"파일 처리 실패 {file_path}: {str(e)}")
...
async def process_single_file(session, file_path, image_data, uuid_str, ocr_server_ip, ocr_server_port):
    """
    단일 파일을 처리하는 함수
    """
    url = f"http://{ocr_server_ip}:{ocr_server_port}/ocr/"
    ...
    try:
        ...
        async with session.post(url, data=form, timeout=30) as response:
            ...
            if response.status == 200:
                ...
                logging.error(f"서버 오류 응답 - {file_path}: {response.status}, 응답: {response_text}")
                return {'success': False, 'data': None}
                
    except Exception as e:
        logging.error(f"요청 실패 - {file_path}: {str(e)}")
        return {'success': False, 'data': None}

def process_batches_in_parallel(filtered_files, json_data, ocr_server_ip, ocr_server_port, batch_size=10):
    """
    여러 배치를 병렬로 처리하는 함수
    """
    # 결과 저장용 변수
    ...
    try:
        ...
        with Pool(processes=num_processes) as pool:
            process_args = [
                (batch, json_data, ocr_server_ip, ocr_server_port)
                for batch in batches
            ]
            
            # 결과 실시간 처리
            for batch_result in pool.imap_unordered(process_batch, process_args):
                if batch_result:
                    all_results.extend(batch_result)
                    batch_success = sum(1 for r in batch_result if r['success'])
                    batch_failure = len(batch_result) - batch_success
                    success_count += batch_success
                    failure_count += batch_failure
                    
                    logging.info(f"진행 상황 - 성공: {success_count}, 실패: {failure_count}")
        
        return all_results, success_count, failure_count
...
def main():
    ...
    try:
        ...
        results, success_count, failure_count = process_batches_in_parallel(
            filtered_files,
            json_data,
            ocr_server_ip,
            ocr_server_port,
        ...
        # 최종 결과 출력
        logging.info(f"총 파일 처리 완료: {success_count}개 성공, {failure_count}개 실패")
